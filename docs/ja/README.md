...
- **React 18 レンダラー**は **globals-on-`window`** モジュール規約に従います。ファイルは `src/main.tsx` から副作用目的でロードされ、`window` に登録されます。`src/main.tsx` 内の順序が重要であり、新しいファイルは `Object.assign(window, { Foo })` で自身を登録する必要があります。
